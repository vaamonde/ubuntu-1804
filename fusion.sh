...
# Kernel >= 4.15.x
# Testado e homologado para a versão do FusionInventory Server 9.5.x, Agent 2.5.x
#
...
# Vídeo de instalação do GLPI Help Desk: 
#
...
#
# Declarando as variáveis de download do FusionInventory (Links atualizados no dia 06/08/2020)
...
FUSIONTASK="https://github.com/fusioninventory/fusioninventory-agent/releases/download/2.5.2/fusioninventory-agent-task-collect_2.5.2-1_all.deb"
...
# opção do comando if: [ ] = testa uma expressão, -d = testa se é diretório existe
echo -e "Verificando se o GLPI Help Desk está instalado, aguarde...\n"
	if [ -e "$GLPI" ]
		then
    		echo "O GLPI Help Desk está instalado, continuando com o script..."
			sleep 5
	else
...
echo -e "Instalando as Dependências do FusionInventory Server e Agent, aguarde..."
	...
	apt -y install dmidecode hwdata ucf hdparm &>> $LOG
	apt -y install perl libuniversal-require-perl libwww-perl libparse-edid-perl &>> $LOG
	apt -y install libproc-daemon-perl libfile-which-perl libhttp-daemon-perl &>> $LOG
	apt -y install libxml-treepp-perl libyaml-perl libnet-cups-perl libnet-ip-perl &>> $LOG
	apt -y install libdigest-sha-perl libsocket-getaddrinfo-perl libtext-template-perl &>> $LOG
	apt -y install libxml-xpath-perl libyaml-tiny-perl &>> $LOG
...
echo -e "Fazendo o download do FusionInventory Agent do site Oficial, aguarde..."
	...
	wget $FUSIONTASK -O task.deb &>> $LOG
...
echo -e "Iniciando o serviço do FusionInventory Agent, aguarde..."
	...
	systemctl reload fusioninventory-agent &>> $LOG
